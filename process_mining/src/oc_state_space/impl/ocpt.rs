...
impl OCPTStateInterface {
    ...
    fn calculate_lb(
        ...
        total_cost + more_epsilon_cost + added_void_cost + added_void_event_cost
    }
...
impl ModelStateInterface for OCPTStateInterface {
    ...
    fn get_initial_node(&mut self, query_case_stats: &CaseStats) -> Self::NodeType {
        OCPNStateNode::new(
            Marking::new(self.model.clone()),
            CaseGraph::new(),
            0.0,
            None,
        ...
        if (node.action_path.last().unwrap().is_pre_firing()) {
            ...
            for place in initial_places {
                ...
                if !higher_types_used {
                    ...
                    let lb =
                        self.calculate_lb(&query_case_stats, &new_partial_case_stats, &new_marking);
